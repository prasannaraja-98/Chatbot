...
#Dictionary with line id and corresponding conversations
id2line={}
for line in lines:
    _line=line.split(' +++$+++ ')
    if(len(_line)==5):
...
#creating a list of all convesations 
conversations_id=[]
...
#cleaning tha answers
clean_answers=[]
for answer in answers:
    clean_answers.append(clean_text(answer))
 
#create a dictionary that maps word to it's occurences
word2count={}
...
# creating two dictionaries that maps question and answers to unique integer
threshold=20
questionswords2int={}
word_number=0
...
# sort questions and answers based on the length of question
...
def decode_training_set(encoder_state,decoder_cell,decoder_embedded_input,sequence_length,decoding_scope,output_function,keep_prob,batch_size):
    attention_states=tf.zeros([batch_size,1,decoder_cell.output_size]) #3d matrix of only zeros like numpy zeros no.of lines =batch_size,no.of columns=1,
...
def decode_test_set(encoder_state,decoder_cell,decoder_embeddings_matrix,sos_id,eos_id,maximum_length,num_words,sequence_length,decoding_scope,output_function,keep_prob,batch_size):
    ...
    test_decoder_function=tf.contrib.seq2seq.attention_decoder_fn_inference(output_function,encoder_state[0],attention_keys,attention_values,attention_score_function,attention_construct_function,
                                                                       decoder_embeddings_matrix,sos_id,eos_id,maximum_length,num_words  ,name="attn_dec_inf")
...
def seq2seq_model(inputs,targets,keep_prob,batch_size,sequence_length,answers_num_words,questions_num_words,encoder_embedding_size,decoder_embedding_size,rnn_size,num_layers,questionswords2int):
    encoder_embedded_input=tf.contrib.layers.embed_sequence(inputs,answers_num_words+1,encoder_embedding_size,initializer=tf.random_normal_initializer(0,1))
...
epochs=100 #is whole process of getting batches of inputs into neural network forward propagating them into encoder to get encoder state 
batch_size=64
rnn_size=512
num_layers=3
encoding_embedded_size=512
decoding_embedded_size=512
learning_rate=0.01
learning_rate_decay=0.9 #by which percentage the learning rate is reduced over the iteration
min_learning_rate=0.0001
keep_probability=0.5

#defining a session on which all the training will take place 
tf.reset_default_graph()
session=tf.InteractiveSession()

#loading the mdoel input
...
training_predictions,test_predictions =seq2seq_model(tf.reverse(inputs,[-1]),
                                                     ...
                                                     encoding_embedded_size,
                                                     decoding_embedded_size,
                                                     rnn_size,
                                                     num_layers,
                                                     questionswords2int)

#setting up the loss error ,the optimiser and gradient clipping gc is a technique that will cap the minimum and maximum value 
...
# splitting the data into batches of qquestions and answers
...
for epoch in range(1,epochs+1):
    ...
    if batch_index % batch_index_check_training_loss == 0:
        print("Epoc: {:>3}/{}, Batch:{:>4},Training Loss Error {:>6.3f},Training time on 100 batches :{:d} seconds".format(epoch,epochs,batch_index,len(training_questions)//batch_size,total_training_loss_error/batch_index_check_training_loss,int(batch_time * batch_index_check_training_loss)))
        total_training_loss_error=0
...
while(True):
    ...
    question+=[questionswords2int['<PAD>']*(20-len(question))]
